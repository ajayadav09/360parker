  ...
  $.ThreeSixty = function (el, options) {
    ...
    base.showNavigation = function() {
      if (AppCongif.navigation && !AppCongif.navigation_init) {
        var nav_bar, next, previous, play_stop,jump1; 
    ...
    base.next = function(event) {
      ...
      AppCongif.endFrame += 4;
	   $('#productnum, #explorebtn').show();
	   $("#nextComp").removeClass("shake");
	   if(AppCongif.endFrame >= 41){
		  ...
		  $("#nextComp").fadeTo("fast", 0.5);
		   $("#productnum p").css({'display':'none'});
		     $(".yellowstrip").css({'display':'none'});
				 $("#popuptrucks, .overlay ").fadeIn(2000);
    ...
    if ( (event.which == leftKeyCode) && (AppCongif.endFrame > 5) )  car.previous() ;
    if ( event.which == rightKeyCode &&  (AppCongif.endFrame > 4) && (AppCongif.endFrame < (AppCongif.totalFrames-2)) ) car.next();
  } ); 
